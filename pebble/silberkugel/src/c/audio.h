...
// Quelle des Streams. Phase 1 kennt nur zwei: einen Dauerton als Last und
// reine Stille. Beides laeuft mit voller Rate, der Transport ist derselbe.
typedef enum {
  AudioSourceTone = 0,
  AudioSourceSilence,
} AudioSource;
...
void audio_set_source(AudioSource src); // Ton oder Stille
AudioSource audio_source(void);
void audio_set_calibrate(bool on);      // leichter Ueberschuss erzwingt Backpressure, eicht die Schaetzung
