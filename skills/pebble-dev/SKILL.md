---
name: pebble-dev
description: Professionelle Entwicklung von Pebble-Watchapps und Watchfaces in C (und Alloy/JavaScript) mit dem Core-Devices-SDK - von der Toolchain über Projektaufbau, Speicherbudget, Plattformunterschiede und AppMessage bis zum kopflosen Test im Emulator. Immer verwenden, wenn eine Pebble-App oder ein Watchface gebaut, erweitert, portiert, aufgeräumt oder repariert werden soll, wenn "pebble build", "pebble install", "pebble new-project", "pebble logs" oder die package.json einer Pebble-App vorkommen, wenn Window, Layer, TextLayer, MenuLayer, ActionBarLayer, GBitmap, AppMessage, persist_write, tick_timer_service, AppTimer, Wakeup, Dictation oder Background Worker auftauchen, bei Abstürzen ("App fault!", PC/LR-Adressen), Speicherproblemen ("Out of memory", zu großer Heap), Emulator-Problemen (hängender qemu-pebble, "Connection refused", "No SDK installed") und bei der Frage, auf welchen Plattformen (aplite, basalt, chalk, diorite, flint, emery, gabbro) etwas läuft und wie man den Unterschied abfängt. Für Tonausgabe stattdessen pebble-audio verwenden, für Store-Einreichung pebble-publish.
---
