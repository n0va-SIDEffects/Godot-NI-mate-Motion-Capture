...
static bool prv_has_recent(void) {
  return model_get()->recent_count > 0;
}
...
static void prv_draw_header(GContext *ctx, const Layer *cell_layer, uint16_t section, void *context) {
  menu_cell_basic_header_draw(ctx, cell_layer, prv_is_recent_section(section) ? "Zuletzt" : "Projekte");
}
...
static void prv_draw_row(GContext *ctx, const Layer *cell_layer, MenuIndex *index, void *context) {
  ...
  if (prv_is_recent_section(index->section)) {
    ...
    menu_cell_basic_draw(ctx, cell_layer,
                         e->description[0] ? e->description : "(ohne Beschreibung)",
                         e->project_name[0] ? e->project_name : "Kein Projekt", NULL);
    prv_draw_color_bar(ctx, cell_layer, e->color);
    return;
  }
  if (index->row == 0) {
    menu_cell_basic_draw(ctx, cell_layer, "Ohne Projekt", "Leeren Timer starten", NULL);
    return;
  }
  ...
  menu_cell_basic_draw(ctx, cell_layer, p->name, NULL, NULL);
